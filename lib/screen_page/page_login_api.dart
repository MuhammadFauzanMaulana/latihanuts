...
import 'package:latihanuts/model/model_login.dart';
...
import 'package:latihanuts/screen_page/page_register_api.dart';
import 'package:latihanuts/utils/session_manager.dart';
...
class _PageLoginTugasState extends State<PageLoginTugas> {
  ...
  Future<ModelLoginTugas?> loginAccount() async {
    //handle error
    try {
      setState(() {
        isLoading = true;
      ...
      http.Response response = await http
          .post(Uri.parse('http://192.168.124.51/edukasi_server/login.php'), body: {
        "username": txtUsername.text,
        "password": txtPassword.text,
      });
      ModelLoginTugas data = modelLoginTugasFromJson(response.body);
      //cek kondisi
      if (data.value == 1) {
